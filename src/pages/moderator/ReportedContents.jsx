    ...
    const result = await Swal.fire({
      ...
      html: (
        <div className="text-left">
          <p className="text-gray-600 mb-4">You are about to permanently delete this reported product:</p>
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-red-600">
                  Reported by: {product.reportedBy?.name || 'Unknown'} • Votes: {product.votes}
                </p>
  ...
  const handleDelete = async (productId) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setReportedProducts(prev => prev.filter(product => product._id !== productId));
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <div className="flex items-center space-x-2 mt-1">
                ...
                <span className="text-xs text-gray-400">({product.owner?.email})</span>
              </div>
            </div>

            {/* Report Information */}
            <div className="bg-red-50 border border-red-200 rounded-xl p-3">
              <strong className="text-red-700">Report Information:</strong>
              <div className="mt-2 space-y-2">
                <div className="flex items-center justify-between text-sm">
                  <span className="text-red-600">Reported By:</span>
                  <span className="font-semibold text-red-700">
                    {product.reportedBy?.name || 'Unknown User'}
                  </span>
                </div>
                {product.reportReason && (
                  <div>
                ...
                {product.reportedAt && (
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-red-600">Reported On:</span>
    ...
    <div className="p-6 sm:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-8">
          <h2 className="text-3xl font-bold text-gray-900 mb-2">Reported Contents</h2>
        ...
        {reportedProducts.length === 0 ? (
          <div className="text-center py-12">
            ...
            <h3 className="text-xl font-semibold text-gray-900 mb-2">No Reported Content</h3>
            <p className="text-gray-600">All clear! No products have been reported.</p>
          </div>
        ) : (
                  ...
                  {reportedProducts.map((product) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center space-x-3">
                          <img
                            src={product.reportedBy?.image || "https://www.w3schools.com/w3images/avatar2.png"}
                            alt={product.reportedBy?.name}
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-xs text-gray-500 truncate">
                              {product.reportedBy?.email || 'No email'}
                            </p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="max-w-xs">
                          <p className="text-sm text-gray-900 line-clamp-2">
                            {product.reportReason || 'No reason provided'}
                          </p>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-500">
                        <div className="space-y-1">
                          <div>{product.reportedAt ? new Date(product.reportedAt).toLocaleDateString() : 'Unknown'}</div>
                          <div className="text-xs text-gray-400">
                            {product.reportedAt ? new Date(product.reportedAt).toLocaleTimeString() : ''}
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
        ...
        {reportedProducts.length > 0 && (
          <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-gradient-to-br from-red-50 to-orange-100 rounded-2xl p-6 text-center border border-red-200">
              <div className="text-2xl font-bold text-red-600">{reportedProducts.length}</div>
            ...
            <div className="bg-gradient-to-br from-purple-50 to-violet-100 rounded-2xl p-6 text-center border border-purple-200">
              <div className="text-2xl font-bold text-purple-600">
                {reportedProducts.reduce((total, product) => total + product.votes, 0)}
              </div>
